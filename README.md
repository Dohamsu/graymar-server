...
| Zod | 4.3 | 런타임 검증 |
| Gemma 4 26B MoE / GPT-4.1 Mini / Gemini | Multi | LLM 내러티브 (OpenRouter 경유) |
...
# LLM 설정 (OpenRouter 경유, 메인 Gemma 4 26B MoE)
LLM_PROVIDER=openai                                  # openai | claude | gemini | mock
OPENAI_API_KEY=sk-or-...                             # OpenRouter API 키
OPENAI_MODEL=google/gemma-4-26b-a4b-it                # 메인 모델
OPENAI_BASE_URL=https://openrouter.ai/api/v1
LLM_ALTERNATE_MODEL=google/gemma-4-26b-a4b-it         # 교차 모델
...
LLM_MAX_TOKENS=1024
LLM_TEMPERATURE=0.8
LLM_JSON_MODE=false                                   # JSON 구조화 출력 (스트리밍과 비호환)
LLM_DIALOGUE_SPLIT=true                               # 2-Stage 대사 분리 파이프라인

# 선택적 LLM 프로바이더
CLAUDE_API_KEY=
GEMINI_API_KEY=
GEMINI_REASONING_MAX_TOKENS=0                         # Gemini Flash thinking 비활성
```
...
```
main.ts → AppModule (12 modules, 95 services, 10 controllers)
├── common/              Guards, Filters, Pipes, Decorators
├── auth/                인증 (JWT 기반 register/login)
├── db/
│   ├── schema/          22 tables (users, runs, turns, battles, memories, bug_reports, ai_turn_logs, entity_facts ...)
│   └── types/           TypeScript types (42+: ServerResultV1, WorldState, NPC, Incident, EndingSummary, EventItemReward ...)
...
│       ├── [Narrative Engine v1 — 9]
│       │   ├── incident-management  Incident 생명주기 (spawn/tick/resolve)
│       │   ├── world-tick           4상 시간 (DAWN→DAY→DUSK→NIGHT, 12tick/day) + soft deadline signal
│       │   ├── signal-feed          5채널 시그널 생성/만료 + generateSoftDeadlineSignal
│       │   ├── operation-session    멀티스텝 LOCATION 세션
│       │   ├── npc-emotional        5축 감정 모델 + posture 자동 계산
│       │   ├── narrative-mark       12종 불가역 서사 표식
│       │   ├── ending-generator     엔딩 조건 체크/결과 생성 + arcRoute 12분기 + personalClosing
│       │   ├── summary-builder      여정 요약 (synopsis/keyEvents/keyNpcs/finale 템플릿 조립)
│       │   └── shop                 상점 메카닉
...
├── runs/                런 생성/조회 API + equipItem/unequipItem/useItem
├── turns/               턴 제출/조회 API (HUB/LOCATION/COMBAT 분기)
├── endings/             여정 아카이브 API (GET /v1/endings, /v1/endings/:runId)
├── llm/                 비동기 LLM 내러티브
│   ├── providers/       OpenAI, Claude, Gemini, Mock 어댑터
│   ├── prompts/         시스템 프롬프트 + PromptBuilder + speechRegister 5종 룰
│   ├── llm-worker       백그라운드 폴러 (PENDING → DONE) + SSE 스트림 브로커
│   ├── context-builder  L0~L4 메모리 컨텍스트 빌드 + deadlineContext 조건부 주입
│   ├── memory-renderer  StructuredMemory → 프롬프트 블록
│   ├── token-budget     블록별 토큰 예산 관리 (총 2500 토큰)
│   ├── mid-summary      6턴 초과 시 초기 턴 200자 요약 압축
│   ├── nano-director    메인 LLM 호출 전 nano 가이드 생성
│   ├── nano-event-director  동적 이벤트 컨셉·NPC·fact 생성
│   ├── dialogue-generator  서술-대사 2-Stage 분리 + 어체 validator (1회 재시도)
│   ├── stream-classifier   narration/dialogue 세그먼트 분류 + paragraphStart
│   ├── lorebook             키워드 트리거 세계 지식 주입
│   └── fact-extractor       entity_facts UPSERT (Memory v4)
├── party/               파티 시스템 (CRUD, SSE 채팅, 로비, 파티 턴)
├── campaigns/           캠페인 구조
├── scene-image/         AI 장면 이미지 (Gemini, rate limit)
├── portrait/            AI 초상화 생성 (Gemini, rate limit)
└── bug-report/          인게임 버그 리포트 (생성/조회/상태 변경, client snapshot + network log 수집)
```
