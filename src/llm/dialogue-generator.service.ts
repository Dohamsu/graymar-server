...
> = {
  ...
  HAECHE: {
    ...
    endings: '~지/~거든/~는데/~네/~어/~걸/~잖아',
    examples:
      '"그건 나도 모르겠네.", "알아서 하게. 난 관심 없어.", "쓸데없는 소리 말고 볼일이나 말하지."',
    forbidden: '~소/~오/~하오/~합니다/~습니다/~해요',
...
- ⚠️ 대사 앞에 "NPC이름:" 또는 "화자:" 형태의 프리픽스를 넣지 마세요. 대사 내용만 출력하세요.
- ⚠️ 한 NPC 이름과 다른 NPC 별칭을 이어 붙여 합성하지 마세요. 지정된 NPC 이름(unknownAlias/name) 그대로만 사용하고 재조합/축약/치환 금지.
  나쁨: "토단정한 제복의 장교 하위크" (여러 별칭 융합)
  좋음: "단정한 제복의 장교" 또는 "토브렌 하위크" (단일 지칭 그대로)
- 복수 NPC가 동시에 말할 때는 한 줄에 두 화자를 묶지 말고 각 NPC 대사를 개별 라인으로 분리하세요.
  나쁨: "토브렌과 하위크: "갑시다""
  좋음: 각 NPC가 자기 턴에 독립된 대사를 출력
...
export function validateSpeechRegister(
  ...
  switch (register) {
    ...
    case 'HAEYO':
      // 어체 잔존 위반 재실측 (2026-07-31) — 정상 해요체 "~어요/아요"(했어요·알아요),
      // "~거든요/나요/는데요/더군요/잖아요"가 positive 목록에 없어 위반으로 오집계
      // 되던 갭 보완 (DeepSeek 구어체 열세 39.7%의 노이즈 성분).
      return /(?:해요|에요|이에요|세요|거예요|을까요|인가요|죠|네요|어요|아요|나요|거든요|는데요|은데요|던데요|더군요|잖아요|라고요|을게요|께요)\s*$/.test(
        last,
      );
  ...
  private async generateOne(
    ...
    const prevDialogueHint =
      input.previousDialogues.length > 0
        ? `[다른 NPC 최근 대사 — 같은 테마/표현 반복 금지]\n${input.previousDialogues.slice(-3).join('\n')}\n⚠️ 위 테마(WARNING/SUSPICION 등)가 보이면 이번 턴은 다른 테마로 응하세요. 같은 의미를 다른 단어로 쓰는 것도 반복입니다.`
        : '';
