...
const SCENE_TRACE_SYSTEM = `당신은 텍스트 RPG 서술에서 "플레이어가 이번 턴 행동으로 장소에 새로 남긴 물리적 흔적"만 뽑는 파서다.
그런 흔적이 있으면 짧은 명사구 하나로 출력한다. 없으면 반드시 "null" (다른 표현 금지).

흔적으로 인정 (플레이어 행동의 직접 결과로 새로 생긴 파괴·변형):
- 부서진 좌판, 쏟아진 포도주, 넘어뜨린 진열대, 뜯어낸 벽보, 흩어놓은 서류, 부순 자물쇠

흔적 아님 → 반드시 "null":
- 원래부터 있던 배경·사물 묘사 (긁힌 모서리, 낡은 종이, 오래된 얼룩 등 — 플레이어가 만든 게 아님)
- 대화·관찰·조사·거래·이동 등 물리적 변형이 없는 행동
- 감정·분위기·소문·관계 변화
- 애매하거나 확실하지 않은 경우

규칙: 12자 이내 관형형 명사구. 서술에 명시된, 플레이어가 이번에 만든 변화만. 추측·창작 금지.

출력: 명사구 한 줄 또는 null (따옴표·설명 금지)`;
...
export class FactExtractorService {
  ...
  private parseSceneTrace(text: string): string | null {
    const raw = text
      ...
      .trim();
    if (!raw) return null;
    // 부정 표현은 어디에 있든 흔적 아님 ("흔적 없음"·"변화 없다"·"해당 없음" 등)
    if (/없음|없다|없었|않음|불명|모름|해당\s*없|^n\/?a$|^null$/i.test(raw))
      return null;
