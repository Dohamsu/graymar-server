...
export function sanitizeNanoChoiceNpcsCore(
  ...
  for (const c of choices) {
    ...
    if (
      !NANO_CHOICE_CONV_AFFORDANCES.has(
        String((payload?.affordance as string | undefined) ?? ''),
      )
    )
      continue;
...
/** Step E — 대사 내부 "NPC이름: " 프리픽스 제거 (긴 이름 먼저 매칭) */
export function removeNpcPrefixInQuotesCore(
  narrative: string,
  allNpcs: NpcNameLike[],
): string {
  const npcNamePatterns = allNpcs
    .flatMap((n) => [n.name, n.unknownAlias, n.shortAlias])
...
/**
 * Step F 본체 — 서술의 첫 @마커 NPC가 primaryNpcId와 다르면 마커+본문을 교정.
 * corrected 가 null 이 아니면 교정이 발생한 것 (호출부가 로깅).
 */
export function fixNpcMismatchCore(
  narrative: string,
 ...
 * Tier 1 정확 일치 → Tier 2 라벨이 이름/별칭 전체 포함 → Tier 3 축약
 * (unknownAlias 마지막 1~2단어). 각 tier에서 후보가 정확히 1명일 때만 승격,
 * 다중 후보면 null (무명 유지 — 오귀속 방지).
 */
export function resolveColonLabelNpcCore<T extends NpcNameLike>(
  alias: string,
  allNpcs: T[],
): T | null {
  const a = alias.trim();
  if (a.length < 2) return null;
  const tiers: Array<(n: T) => boolean> = [
    // Tier 1: 정확 일치
    (n) =>
      a === n.name ||
      a === n.unknownAlias ||
...
/**
 * 카드-서술 정합 (2026-07-11) — 서술 본문에 NPC 이름/별칭 언급 여부.
 * 결함 B: includes 는 "토브렌" 속 '브렌'처럼 다른 이름의 부분 문자열에
 * 오매칭. 앞 경계(한글 아님)만 요구 — 한국어는 이름 뒤에 조사가 붙으므로
 * 뒤 경계는 걸지 않는다.
 */
export function npcMentionedInNarrativeCore(
  narrative: string,
  def: NpcNameLike | undefined,
): boolean {
  if (!def) return false;
  const names = [
    def.name,
 ...
 * 알려진 unknownAlias 직전에 한글 1~2자가 밀착해 있으면(앞 경계: 문두/공백/
 * 문장부호) 접두를 제거해 별칭만 남긴다. 정상 문장("뭔가 수상한 창고 관리인")은
 * 접두 앞에 공백이 있어 걸리지 않는다.
 */
export function stripFusedAliasPrefixCore(
  narrative: string,
  aliases: Array<string | null | undefined>,
  onFix?: (fused: string, alias: string) => void,
): string {
  for (const alias of aliases) {
    if (!alias || alias.length < 4 || !narrative.includes(alias)) continue;
    const esc = alias.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const re = new RegExp(`(^|[\\s"“”'')(.,!?…])([가-힣]{1,2})(${esc})`, 'g');
    narrative = narrative.replace(re, (_match, pre, fused, aliasHit) => {
      onFix?.(fused as string, alias);
      return `${pre}${aliasHit}`;
    });
  }
  return narrative;
...
/**
 * 완주 평가 ③ (2026-07-11) — 별칭 조각 + 실명 융합 정규화.
 * 별칭 선행 단어(1~2개) + 실명 밀착 → 실명만 남긴다.
 * (실측: "수상한 토브렌 하위크" = unknownAlias 앞단어 + name)
 */
export function stripAliasFragmentBeforeNameCore(
  narrative: string,
  npcs: NpcNameLike[],
  onFix?: (frag: string, name: string) => void,
): string {
...
/**
 * 완주 평가 ③ (2026-07-11) — NPC 이름/별칭 뒤 주격·목적격 조사 교정.
 * 이름 직후 조사+경계 패턴만 대상이라 오탐 여지 최소 — 받침 규칙에 맞는
 * 조사로만 치환. (실측: "하위크이 갑자기", "회계사이 익숙한")
 */
export function fixNpcNameParticlesCore(
  narrative: string,
  npcs: NpcNameLike[],
): string {
...
export class LlmWorkerService implements OnModuleInit, OnModuleDestroy {
  ...
  private async reconcileSpeakingNpcAndPortrait(params: {
    narrative: string;
    serverResult: ServerResultV1;
    appearedNpcIds: Set<string>;
    portraits: Record<string, string>;
    ...
    //    최후단으로 이동. 원래 5.9 주석은 역사 참조용으로 유지)
    {
      ...
      // appearedNpcIds에 안 잡혀 "실제 등장 NPC로 카드 교체" 로직이 죽어 있었다
        ...
        if (
          ...
          if (resolvedNpcId) {
            ...
            if (ctx.primaryNpcId == null) {
              ...
              await this.applyRunStatePatch(pending.runId, 'LockSeed', (rs) => {
      ...
      if (existingPortrait && appearedNpcIds.size > 0) {
        if (
          existingPortrait.npcId &&
          !appearedNpcIds.has(existingPortrait.npcId)
        ) {
          const newPortraitNpc = [...appearedNpcIds].find(
            (id) => portraits[id],
          );
          if (newPortraitNpc) {
            this.logger.debug(
              `[CardResolve] turn=${pending.turnNo} 카드 교체: ${existingPortrait.npcId} → ${newPortraitNpc} (appeared=${[...appearedNpcIds].join(',')})`,
            );
            const npcDef = this.content.getNpc(newPortraitNpc);
            const npcState = npcStatesRef?.[newPortraitNpc];
            ui.npcPortrait = {
              npcId: newPortraitNpc,
              npcName:
                npcState && npcDef && getNpcDisplayNameFn
                  ? getNpcDisplayNameFn(npcState, npcDef, pending.turnNo)
                  : (npcDef?.name ?? newPortraitNpc),
              imageUrl: portraits[newPortraitNpc],
              isNewlyIntroduced:
                npcState?.introduced &&
      ...
      } else if (existingPortrait && appearedNpcIds.size === 0) {
        ...
        // 정본: npcMentionedInNarrativeCore (결함 B — 앞 경계 오매칭 방지)
        const mentionedInNarrative = (npcId: string): boolean =>
          npcMentionedInNarrativeCore(narrative, this.content.getNpc(npcId));
        const keepCard =
          !!expectedNpcId &&
      ...
      if (srChanged) {
        ...
        await this.db
          .update(turns)
          .set({
            serverResult: updatedSr as unknown as ServerResultV1,
          })
...
${npcList}`,
  ...
  private async processTurnInner(
    ...
    try {
      ...
      if (callResult.success && callResult.response && !isMockFallback) {
        ...
        // [arch/77 P4.1] 서술 품질 후처리 체인 — narrative-filter.core 정본 호출.
        // 순서·시맨틱 동일(플레이어 대사 방어→P1→P1b→R1→P4→P5→P6→중복 제거),
        // llmChoices label은 제자리 변조.
        {
          const rs = runSession?.runState as
            | Record<string, unknown>
            | undefined;
          const filterOut = applyNarrativeQualityFilters(narrative, {
            approachRules: this.content.getTextReplacements().npcApproach,
            npcStates: rs?.npcStates as
              | Record<string, { introduced?: boolean }>
              | undefined,
            getNpc: (id) => this.content.getNpc(id),
            llmChoices,
            directorOpening: directorHint?.opening ?? null,
            jsonModeParsed,
          });
          narrative = filterOut.narrative;
          if (filterOut.violations.length > 0) {
            this.logger.warn(
              `[NarrativeFilter] turn=${pending.turnNo} violations: ${filterOut.violations.join(' | ')}`,
            );
          }
        }
      } else {
      ...
      if (narrative && runSession?.runState) {
        ...
        if (primaryNpcIdForFarewell && farewellNpcDef) {
          const { isNpcFarewellUtterance } =
            await import('../common/dialogue-act.js');
      ...
      // [arch/77 P4.4] 5.13 speakingNpc·npcPortrait 최종본 재결정 —
      // reconcileSpeakingNpcAndPortrait로 추출. 완전형 마커 역해석이
      // appearedNpcIds(Set)를 제자리 보충, serverResult UPDATE·LockSeed CAS는
      // 원 구조 그대로 내부 보존.
      await this.reconcileSpeakingNpcAndPortrait({
        narrative,
        serverResult,
        appearedNpcIds: _appearedNpcIds,
        portraits: _portraits,
        npcStatesRef: _npcStatesRef,
        getNpcDisplayNameFn: _getNpcDisplayNameFn,
        focusedNpcId: llmContext.focusedNpcId ?? null,
        pending,
      });
  ...
  private tryParseJson(raw: string): NarrativeJsonOutput | null {
    try {
      ...
      const parsed = JSON.parse(cleaned) as {
        segments?: Array<{ type?: string; speaker_id?: string; text?: string }>;
      };
      ...
      return parsed as unknown as NarrativeJsonOutput;
    } catch {
      return null;
    }
  }
  ...
  private stripFusedAliasPrefix(narrative: string): string {
    // 정본: stripFusedAliasPrefixCore
    const allNpcs = this.content.getAllNpcs?.() ?? [];
    return stripFusedAliasPrefixCore(
      narrative,
      allNpcs.map((npc) => npc.unknownAlias),
      (fused, alias) =>
        this.logger.log(
  ...
  private stripAliasFragmentBeforeName(narrative: string): string {
    // 정본: stripAliasFragmentBeforeNameCore
    return stripAliasFragmentBeforeNameCore(
      narrative,
      this.content.getAllNpcs(),
      (frag, name) =>
        this.logger.debug(
          `[AliasFragFix] "${frag} ${name.slice(0, 10)}…" → 실명 정규화`,
        ),
    );
  }
  ...
  private fixNpcNameParticles(narrative: string): string {
    // 정본: fixNpcNameParticlesCore
    return fixNpcNameParticlesCore(narrative, this.content.getAllNpcs());
  }
  ...
  private async applyRunStatePatch(
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      const updated = await this.db
        ...
        .set({
          runState:
            working as unknown as import('../db/types/permanent-stats.js').RunState,
        })
        .where(
          and(
