...
import { getNpcCurrentActivity } from '../db/types/npc-schedule.js';
...
/**
 * arch/69 B1 — NPC 자기 맥락 (immediateGoal 정보 편향 해소용).
 * 순수 함수로 조립해 startNpcReaction 인라인이 아닌 유닛 테스트 가능한
 * 형태로 유지 (테스트 감사 2026-07-12 패턴).
 */
export interface NpcSelfContext {
  currentActivity: string | null;
  selfAgenda: string | null;
  dialogueAct: string | null;
}

export function buildNpcSelfContextCore(input: {
  ...
  return {
    currentActivity: getNpcCurrentActivity(input.schedule, input.phase),
    selfAgenda: input.agenda?.trim() ? input.agenda.trim() : null,
...
export interface NpcReactionContext {
  ...
  /**
   * arch/69 B1 — NPC 자기 목적/활동. immediateGoal 이 "정보 탐색" 일변도로
   * 수렴하지 않도록, LLM 에 "이 NPC 는 지금 뭘 하는 중이고 뭘 원하는가"를
   * 데이터로 제공한다. (baseline: 정보 편향 88%·자기목적 0%)
   */
  currentActivity?: string | null; // schedule 현재 4상 활동
  selfAgenda?: string | null; // 콘텐츠 agenda (장사·경계·평화 등 자기 목적)
  /** 사교 발화 분류 (GREETING/WELLBEING/THANKS/FAREWELL). 사교 턴 → 자기 삶 반응 유도. */
  dialogueAct?: string | null;
...
immediateGoal: NPC가 지금 원하는 것 (15~30자, 의도만)
  ⚠️ 기본값은 자기 삶이다 — 주어진 [지금 하는 일]을 계속하거나 [자기 목적]을
     이루려 한다. 플레이어가 정보를 파고들 때(INVESTIGATE/PERSUADE·캐묻는 질문)
     만 정보 관련 목표를 둔다. 이방인의 인사·잡담(대화 성격이 사교 발화)에
     NPC가 먼저 정체를 떠보거나 정보를 캐내려 들지 마라 — 그건 부자연스럽다.
  올바른 형태(자기목적, 기본): "손님 접대하며 하던 일 잇기", "평화로운 장사 유지",
     "경계 늦추지 않고 거리 두기", "빚 걱정에 딴생각"
  올바른 형태(정보, 플레이어가 캐물을 때만): "신원을 캐묻지 않게 화제 돌리기"
...
export class NpcReactionDirectorService {
  ...
  private buildUserMessage(ctx: NpcReactionContext): string {
    ...
    // arch/69 B1 — 사교 발화면 명시. "물음이 아니라 인사·잡담" 신호로
    // immediateGoal 을 정보 추출이 아닌 자기 삶 반응 쪽으로 유도.
    if (ctx.dialogueAct)
      parts.push(`대화 성격: ${ctx.dialogueAct} (사교 발화 — 정보 심문 아님)`);
