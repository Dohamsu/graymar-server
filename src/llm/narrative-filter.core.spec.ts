...
describe('detectMissingDialogue — 대사 누락 판정 (bug 5bcfe78b)', () => {
  ...
  // 2026-07-24 오탐 리뷰 — 고/자/는데는 정상 종결과 충돌해 ①에서 제외.
  // 발화형 reaction 턴이면 ②가 꼬리 형태와 무관하게 인수한다.
  it('오탐 방지: 청유형 종결(…돌아가자.) + reaction 없음 → null', () => {
    expect(detectMissingDialogue('이제 그만 돌아가자.', null)).toBeNull();
  });

  it('오탐 방지: 명사 종결(…낡은 창고. / …혼자.) + reaction 없음 → null', () => {
    expect(
      detectMissingDialogue('그가 가리킨 곳은 낡은 창고.', null),
    ).toBeNull();
    expect(detectMissingDialogue('이 밤, 그는 혼자.', undefined)).toBeNull();
  });

  it('오탐 방지: 구어 여운형 종결(…심상치 않은데.) + reaction 없음 → null', () => {
    expect(detectMissingDialogue('분위기가 심상치 않은데.', null)).toBeNull();
  });

  it('고/자/는데 절단은 발화형 reaction이면 ②(REACTION_NO_DIALOGUE)가 인수', () => {
    expect(
      detectMissingDialogue('그가 가리킨 곳은 낡은 창고.', 'OPEN_UP'),
    ).toBe('REACTION_NO_DIALOGUE');
    expect(detectMissingDialogue('그는 손을 뻗어 문을 열고.', 'PROBE')).toBe(
...
describe('trimDanglingConnectivePunct — 연결어미 매달린 종결부호 정리', () => {
  ...
  // 2026-07-24 오탐 리뷰 후속
  it('꼬리 패턴 공유: 따옴표 파편 포함(…숙이며". → …숙이며)도 정리', () => {
    expect(trimDanglingConnectivePunct('고개를 살짝 숙이며".')).toBe(
      '고개를 살짝 숙이며',
    );
    expect(trimDanglingConnectivePunct('고개를 숙이며 .')).toBe(
      '고개를 숙이며',
    );
  });

  it('청유형 등 trim 어미 세트 밖 정상 종결은 보존(…돌아가자.)', () => {
    expect(trimDanglingConnectivePunct('이제 그만 돌아가자.')).toBe(
      '이제 그만 돌아가자.',
    );
    expect(trimDanglingConnectivePunct('그곳은 낡은 창고.')).toBe(
      '그곳은 낡은 창고.',
    );
  });
});
