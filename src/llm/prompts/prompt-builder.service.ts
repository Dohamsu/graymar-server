...
export class PromptBuilderService {
  ...
  buildNarrativePrompt(
    ...
    if (isPartyMode && ctx.partyActions) {
      // 프리셋 설명은 콘텐츠 정본(활성 팩 presets.json)에서 조회 (불변식 45 —
      // 구 graymar 6종 하드코딩은 타 팩 프리셋(KH_* 등)에서 빈 설명이 됨)
      const memberLines = ctx.partyActions.map((a) => {
        const preset = a.presetId
          ? this.content.getPreset(a.presetId)
          : undefined;
        const desc = preset ? `${preset.name}. ${preset.description}` : '';
    ...
    memoryParts.push(this.buildAcquiredItemsBlock(sr));
    ...
    if (ctx.focusedNpcId && !isCombat) {
      ...
      const focusedDisplay = focusedDef
        ...
        : '이 NPC';
      // [arch/79 2차 재압축 2026-07-27] 인용 예시·중복 부연 제거 (821→~350자).
      //   인용 예시는 불변식 50 anchor 위험도 겸했다. 하드 규칙(1인 발화·단역
      //   비반복·advance-or-dismiss·로테이션 풀)은 전부 보존.
      const auxBlockLines = [
        `[1인 응답 강제 — 보조 NPC 끼어들기 금지]`,
        `⚠️ 이 턴은 ${focusedDisplay} 한 명만 말합니다. 다른 NPC·배경 인물의 따옴표 대사 절대 금지 — 주변 인물의 행동·시선은 서술 1문장 이내로만.`,
        `- 직전 턴의 익명 배경 인물을 같은 신원·소품으로 재등장시키지 마시오. 필요하면 풀(어부/행상/견습공/마부/청소부/술꾼/노점상/짐꾼/사제 보조/음유시인/약초 채집인)에서 매 턴 다른 신원 1문장, 또는 환경(빛·소리·냄새) 묘사로 대체.`,
        `- 지켜보기만 하는 무명 인물(감시자·책임자 등)은 이번 턴 반드시 진전시키시오 — 말을 걸거나, 개입하거나, 퇴장. 동일한 정적 응시 묘사 2회+ 금지.`,
      ];
      if (ctx.recentAuxSpeakers && ctx.recentAuxSpeakers.length > 0) {
        auxBlockLines.push(
          `- 직전 턴 발화한 보조 인물(${ctx.recentAuxSpeakers.join(', ')})은 이번 턴 침묵 (2턴 연속 발화 금지).`,
        );
      }
      // 직전 3턴 본문에 마커 밖으로 등장한 익명 배경 인물 신원(직군/소품) hard 차단.
      if (ctx.recentAuxIdentities && ctx.recentAuxIdentities.length > 0) {
        auxBlockLines.push(
          `- 사용 금지 단역 신원/소품 (직전 3턴 기등장): ${ctx.recentAuxIdentities.join(', ')} — 의미적 변형 재사용도 금지, 풀의 다른 신원이나 환경 묘사로 대체.`,
        );
      }
      ...
      const reactionLines = [
        '[집중 NPC 반응 지시]',
        `${focusedDisplay}가 이번 턴 반응의 중심입니다 (행동: ${focusedAction}${outcome ? ` / 판정: ${outcome}` : ''}).`,
        '- 첫 문장부터 이 NPC의 구체 반응(표정·몸짓·침묵·말투 변화)으로 시작 — 플레이어 행동 재설명 금지, 이 NPC가 무엇을 알아차렸고 무엇을 숨기는지 보여줄 것. 최소 1회 이 NPC 별칭으로 대사를 쓸 것.',
      ];
      if (sceneFrame) {
    ...
    if (!ctx.npcRevealableFact && ctx.factWithheldHint) {
      ...
      factsParts.push(
        [
          ...
          // LLM 원칙(어구 예시 금지): 구체 표현/제스처를 지시하지 않는다 —
          // 인용 어구는 매 턴 그대로 복제되고, 특정 제스처 지정은 anchor가 된다.
          `- 즉답을 피하고 말을 아끼는 반응 — 표현 방식은 NPC의 성격·말투에서 도출`,
          `- 짧은 유보로 화제를 미루는 대사 1마디 이내 (NPC 어체 준수)`,
    ...
    if (!isCombat && !isHub) {
      ...
      if (anonymousPrimaryTurn) {
        factsParts.push(
          `[무명 화자 프레이밍]\n이 장면엔 이름을 밝힐 만한 특정 인물이 없습니다. 사건·단서 정보를 전할 때 한 사람의 길고 상세한 증언으로 풀어놓지 마세요. 우선순위: ① 서술형 소문 — 따옴표 대사 없이 "시장 곳곳에서 ~라는 수군거림이 떠돈다" 식으로 공기 중의 소문·주워들은 말 조각으로 녹이기(무명 @마커 없이). ② 부득이 대사가 필요하면 지나가는 인물 한 명의 짧은 한두 마디만. ⚠️ 무명 인물 여러 명이 번갈아 따옴표 대사를 치는 장면 금지 — 익명 화자 마커는 이 턴 최대 1개.`,
        );
      }
    }
    ...
    if (sr.choices.length > 0) {
      ...
      // [arch/79 2차 재압축 2026-07-27] 규칙 4줄→1줄 (의미 보존)
      const choiceParts = [
        '[참고 선택지] — 서술 범위 경계',
        '엔진 생성 기본 선택지 — 서술에 포함하지 말고, 해당 행동을 서술에서 미리 수행하지 말 것. [CHOICES] 태그 생성 금지 (서버 담당). 서술만 작성.',
        '',
        choiceTexts.join('\n'),
      ];
  ...
  private buildNpcPostureBlocks(
    ...
    if (!isHub && ctx.npcPostures && Object.keys(ctx.npcPostures).length > 0) {
      ...
      factsParts.push(
        [
          // [arch/79 2차 재압축 2026-07-27] 인용 예시 제거·문면 압축 (규칙 보존)
          '[NPC 대화 자세]',
          '대사·행동을 아래 태도에 반드시 일치시킬 것 (태도와 어긋난 호의·정보 제공 금지). agenda는 배경 동기 — 직접 언급은 대화 3회 중 1회 이하.',
          '⚠️ 같은 NPC의 연속 턴 유사 발언 반복 금지 — 턴마다 다른 화제(자기 사정·관찰·경험·질문·침묵). 각 NPC 고유의 말투·호칭만 적용, 다른 NPC의 호칭 차용 금지.',
          '',
          postureLines.join('\n'),
        ].join('\n'),
      );
  ...
  private buildPlayerActionBlocks(
    ...
    if (rawInput && inputType !== 'SYSTEM') {
      if (inputType === 'ACTION') {
        ...
        // [arch/79 2차 재압축 2026-07-27] turnMode 보강 지시 압축 (규칙 보존)
        if (turnMode === 'PLAYER_DIRECTED') {
          parts.push(
            '\n⚠️ [플레이어 주도 장면] 플레이어가 지목한 NPC/대상이 중심 — 다른 NPC가 끼어들거나 장면을 가로채지 말 것. 돌발 이벤트가 아니라 플레이어 행동과 판정 결과에 대한 자연스러운 반응을 서술.',
          );
        } else if (turnMode === 'CONVERSATION_CONT') {
          parts.push(
            '\n⚠️ [대화 연속 장면] 이전 턴에서 이어지는 대화 — 첫 만남처럼 굴지 말고 맥락을 이어 점점 깊어질 것, 이미 다룬 주제 반복 금지.',
            '⚠️ 단일 NPC 응답 강제 (R6): 잠금된 한 NPC만 말한다. 배경 인물의 따옴표 대사 금지 (서술로만). 예외: 이 장면에 처음 등장하는 주요 인물의 극적 첫 대사 1회 — 이후 초점은 그 인물로.',
          );
        }
    ...
    if (ctx.currentTimePhase) {
      ...
      // 불변식 50: 산문형 힌트 금지 — 구 힌트("해가 기울어 그림자가 길어짐.
      // 가로등이 하나둘 켜짐")를 LLM이 서술 도입부로 축자 복제, DUSK 체류 중
      // 5턴 연속 동일 개시어 실측 (2026-07-27, 실유저 런). [시간대 전환] 블록은
      // 2026-07-25에 같은 이유로 예문을 제거했으나 이 상시 블록이 잔존했다.
      // 전보체 광원 정보만 남긴다 (서술로 읽히지 않는 형태).
      const phaseHint: Record<string, string> = {
        DAWN: '광원: 여명. 인적 드묾, 찬 공기.',
        DAY: '광원: 대낮 자연광. 거리 활동 활발.',
        DUSK: '광원: 저무는 빛 + 점등되는 인공조명.',
        NIGHT: '광원: 달빛·등불 등 인공조명 위주.',
      };
      ...
      factsParts.push(
        `[현재 시간대] ${phaseKr} (${phase})\n` +
          ...
          `${contradictionRule}\n` +
          `- 시간 전환이 필요하면 전환을 알리는 한 문장을 먼저 명시.\n` +
          `- 이번 턴에 시간대가 바뀌지 않았다면 도입부를 하늘·조명 묘사로 시작하지 마세요 (직전 턴에서 이미 묘사됨).`,
      );
  ...
  private buildMemoryContextBlocks(
    ...
    if (ctx.protagonistBackground) {
      // [arch/79 2차 재압축 2026-07-27] 매 턴 반복되던 공통 가이드 4줄(예시 어구
      //   포함 — 불변식 50 anchor 위험 겸)을 1줄로. 프리셋 데이터는 무손실.
      memoryParts.push(
        `[주인공 배경]\n${ctx.protagonistBackground}\n` +
          '배경은 직접 설명·독백 회상 금지 — 몸짓·습관·관점·말투로만 행동 묘사에 은연중 스며들게 하세요.',
      );
    }
    ...
    if (!ctx.conversationLock) return factsParts; // 호출 게이트와 이중 방어
    {
      ...
      if (!focusedTakesSingleResponseDuty) {
        lockLines.push(
          ...
          `- 다른 배경 인물(경비·행인·점원·군중)의 따옴표 대사는 여전히 금지. 끼어들기 금지.`,
          `- 주변 인물은 서술(narration)로만 묘사 (예: "옆에서 누군가 지나간다"). 그들의 대사를 따옴표로 쓰지 마세요.`,
          `- 특히 "이 시간에 무엇을 하고 계십니까?", "두 분의 대화가..." 같은 일반 끼어들기를 매 턴 반복하면 게임이 깨집니다.`,
          `- 단, 이 장면에 처음 등장하는 주요 인물이 극적으로 들어오는 경우 그 인물의 첫 대사 1회는 허용하며, 이후 대화의 초점은 그 인물로 옮겨갑니다.`,
        );
      }
  ...
  private buildNpcReactionBlock(
    ...
    // [arch/79 2차 재압축 2026-07-27] 위반 예시 나열 축약 (P3-B 방식 재적용)
    lines.push(
      '',
      '⚠️ NPC 대사·행동·태도·표정 모두 위 결정과 일치 — 반대 방향(회피 결정인데 정보 제공, 침묵인데 긴 대사, 단호한 거절인데 동의) 절대 금지.',
    );
    ...
    if (hasToneFields) {
      ...
      lines.push(
        '⚠️ 위 톤을 외면적 묘사로 표현하되 어휘는 자유롭게 — 시그니처·직전 어구 반복 금지, 같은 분위기도 매 턴 다른 단어·제스처로.',
      );
    }
  ...
  /**
   * [이번 턴 획득 아이템] 블록 — 서버 확정 획득 아이템·장비·골드 목록.
   * LLM 이 구체 아이템 증여를 서술할 때 유일한 허용 목록(환각 방지). 항상 1개 문자열 반환.
   * arch/77 P1.1 — buildNarrativePrompt 에서 추출 (동작 보존).
   */
  private buildAcquiredItemsBlock(sr: ServerResultV1): string {
