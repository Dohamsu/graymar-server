  ...
  buildNarrativePrompt(
    ...
    if (allNpcs.length > 0) {
      ...
      memoryParts.push(
        [
          '[등장 가능 NPC 목록 — 참조용]',
          '⚠️ 절대 규칙: 이 턴에서 등장하는 NPC는 **반드시 1명만**입니다. 2명 이상 동시 등장/대화는 엄격히 금지합니다.',
          '- 플레이어가 특정 NPC에게 행동했으면 → 그 NPC만 등장합니다.',
          '- [이번 턴 NPC가 공개할 정보] 블록이 있으면 → 해당 NPC가 직접 정보를 전달합니다. 다른 NPC가 대신 전달하면 안 됩니다.',
          '- NPC를 지정하지 않은 행동이면 → 상황에 가장 적합한 1명만 고르세요.',
