...
export class PromptBuilderService {
  ...
  private buildNpcPostureBlocks(
    ...
    if (!isHub && ctx.npcPostures && Object.keys(ctx.npcPostures).length > 0) {
      ...
      factsParts.push(
        [
          '[NPC 대화 자세]',
          '이 장소 NPC들의 태도 — 대사·행동은 반드시 아래 태도에 맞출 것 (CAUTIOUS의 자발적 정보 제공, HOSTILE의 호의 등 불일치 금지). agenda는 배경 동기 — 직접 언급은 대화 3번 중 1번 이하, 나머지는 상황 반응·감상·평가로.',
          '⚠️ 같은 NPC가 연속 턴에 비슷한 말("조심하시오" 류) 반복 금지 — 턴마다 다른 화제(자기 사정·관찰·경험·질문·침묵). 각 NPC의 "말투" 특성(더듬기·비유·한숨 등)과 호칭을 반드시 적용하고, 다른 NPC의 고유 호칭(예: "그대")을 빌려 쓰지 말 것.',
          '',
          postureLines.join('\n'),
        ].join('\n'),
      );
      ...
      if (inputType === 'ACTION') {
        ...
        if (turnMode === 'PLAYER_DIRECTED') {
          ...
          parts.push(
            ...
            '⚠️ 단일 NPC 응답 강제 (architecture/51 §B R6): 이 턴은 잠금된 한 NPC만 말합니다. 다른 배경 인물(경비·행인·점원·군중)의 따옴표 대사는 여전히 금지하며 서술로만 묘사하세요. 단, 이 장면에 처음 등장하는 주요 인물이 극적으로 들어오는 경우 그 인물의 첫 대사 1회는 허용하며, 이후 대화의 초점은 그 인물로 옮겨갑니다.',
          );
        }
  ...
  private buildNpcReactionBlock(
    ...
    lines.push(
      '',
      '⚠️ 서버 사전 판단 — NPC 대사·행동·태도·표정이 모두 위 결정과 일치해야 합니다. 반대 방향 절대 금지 (예: DEFLECT/DISMISS인데 핵심 정보를 답하기, SILENCE인데 긴 대사, 거절 FIRM/HOSTILE인데 모호한 동의, immediateGoal과 정반대 전개).',
    );
