...
import type { EventDefV2, WorldState, ArcState, ParsedIntentV2, PlayerAgenda, IncidentRoutingResult } from '../../db/types/index.js';
...
describe('EventDirectorService', () => {
  ...
  function makeEvent(overrides: Partial<EventDefV2> & { stages?: string[]; eventCategory?: string } = {}): EventDefV2 {
    return {
      eventId: 'EVT_TEST',
  ...
  it('기본 선택: 이벤트가 정상 반환됨', () => {
    ...
    const result = director.select(events, 'LOC_MARKET', baseIntent, baseWs, baseArcState, baseAgenda, {}, 1, fakeRng, [], null);
  ...
  it('priority 리매핑: eventCategory가 있는 이벤트의 weight 변경', () => {
    const events = [
      makeEvent({ eventId: 'EVT_HIGH', priority: 8, weight: 10, eventCategory: 'conflict' }),
    ];
    const result = director.select(events, 'LOC_MARKET', baseIntent, baseWs, baseArcState, baseAgenda, {}, 1, fakeRng, [], null);
    expect(result.selectedEvent).not.toBeNull();
  });
  ...
  it('이벤트 없음 → null 반환', () => {
    const result = director.select([], 'LOC_MARKET', baseIntent, baseWs, baseArcState, baseAgenda, {}, 1, fakeRng, [], null);
  ...
  it('EventMatcher 위임: routingResult 전달', () => {
    ...
    const result = director.select(events, 'LOC_MARKET', baseIntent, baseWs, baseArcState, baseAgenda, {}, 1, fakeRng, [], routing);
    expect(result.selectedEvent).not.toBeNull();
  });
  ...
  it('filterLog에 디버깅 출력 포함', () => {
    ...
    expect(result.filterLog.some((log) => log.includes('최종 후보'))).toBe(true);
  });
});
