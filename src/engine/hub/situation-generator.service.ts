...
export class SituationGeneratorService {
  ...
  generate(
    ...
    const incident = this.tryIncidentDriven(ws, locationId, intent, allEvents, incidentDefs);
  ...
  private tryLandmark(
    ...
    for (const event of arcHints) {
      ...
      // arcRouteTag 필터
      if (event.arcRouteTag && ws.mainArc?.activeArcId !== event.arcRouteTag) continue;
  ...
  private tryIncidentDriven(
    ...
    for (const runtime of ws.activeIncidents) {
      ...
      if (relatedEvents.length > 0) {
        ...
        // Incident 관련 NPC 찾기
        const presentNpcs = ws.locationDynamicStates?.[locationId]?.presentNpcs ?? [];
        const relatedNpcs = (def as Record<string, unknown>)['relatedNpcIds'] as string[] | undefined;
        const primaryNpc = relatedNpcs?.find((n) => presentNpcs.includes(n));

        // 관련 facts
        const facts = this.worldFact.findByTags(ws, [runtime.incidentId.toLowerCase()]);
  ...
  private buildEnvironmentalSituation(
    ws: WorldState,
    locationId: string,
    conditions: Array<{ id: string; source: string; effects: Record<string, unknown> }>,
    allEvents: EventDefV2[],
  ): Situation | null {
    ...
    const conditionDescriptions: Record<string, string> = {
      CURFEW: '야간통행금지가 선포되어 거리가 텅 비어 있다. 순찰대의 횃불만이 움직인다.',
      LOCKDOWN: '경비대가 지역을 봉쇄했다. 검문소가 세워지고 모든 출입이 통제된다.',
      FESTIVAL: '축제 분위기가 거리를 감싸고 있다. 음악과 웃음소리가 울려퍼진다.',
      BLACK_MARKET: '어둠 속에서 은밀한 거래의 기척이 느껴진다.',
      RAID_AFTERMATH: '최근 급습의 흔적이 남아있다. 부서진 상자와 흩어진 물건들.',
    };

    const sceneFrame = conditionDescriptions[condition.id] ??
