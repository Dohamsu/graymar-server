...
const KIND_GOAL_AFFINITY: Partial<
  Record<IncidentKind, Partial<Record<IntentGoalCategory, number>>>
> = {
...
export class IncidentRouterService {
  ...
  route(
    ...
    const activeIncidents = (ws.activeIncidents ?? []).filter(
      (i) => !i.resolved,
    );
    ...
    for (const incident of activeIncidents) {
      ...
      const vectorMatch = this.findVectorMatch(
        vectors,
        intentV3.approachVector,
      );
      if (vectorMatch) {
        score +=
          30 + (vectorMatch.preferred ? 15 : 0) - vectorMatch.friction * 5;
      ...
      if (intentV3.secondaryApproachVector) {
        const secMatch = this.findVectorMatch(
          vectors,
          intentV3.secondaryApproachVector,
        );
        if (secMatch) {
      ...
      const affinity =
        KIND_GOAL_AFFINITY[incident.kind]?.[intentV3.goalCategory] ?? 0;
      score += affinity;
  ...
  private findVectorMatch(
    ...
    return (
      vectors.find((v) => v.enabled && v.vector === approachVector) ?? null
    );
  }
