...
export class SignalFeedService {
  ...
  generateFromActionResult(
    ...
    const severityBoost = (actionType === 'FIGHT' || actionType === 'THREATEN' || actionType === 'STEAL') ? 1 : 0;
    const severity = Math.min(5, baseSeverity + severityBoost) as 1 | 2 | 3 | 4 | 5;

    const textMap: Record<string, Record<string, string>> = {
      FIGHT: { SUCCESS: '근처에서 격투 소리가 들렸다.', PARTIAL: '난투극이 벌어진 흔적이 보인다.', FAIL: '크게 소란이 일었다.' },
      THREATEN: { SUCCESS: '누군가 위협을 받았다는 소문이 돈다.', PARTIAL: '언쟁이 벌어진 듯하다.', FAIL: '위협이 역효과를 냈다는 이야기가 퍼진다.' },
      STEAL: { SUCCESS: '물건이 감쪽같이 사라졌다.', PARTIAL: '수상한 움직임이 목격되었다.', FAIL: '도둑이 잡혔다는 소문이 돈다.' },
      TRADE: { SUCCESS: '거래가 성사된 소식이 들린다.', PARTIAL: '흥정 소리가 들린다.', FAIL: '거래가 무산되었다.' },
      BRIBE: { SUCCESS: '뒷거래 소문이 돈다.', PARTIAL: '금전 거래가 오갔다는 이야기가 있다.', FAIL: '뇌물이 거부되었다는 소문이 퍼진다.' },
      TALK: { SUCCESS: '대화가 오갔다.', PARTIAL: '대화가 어색하게 끝났다.', FAIL: '대화가 결렬되었다.' },
      PERSUADE: { SUCCESS: '누군가 설득당한 듯하다.', PARTIAL: '논쟁이 벌어진 듯하다.', FAIL: '설득이 실패했다는 이야기가 들린다.' },
      HELP: { SUCCESS: '누군가 도움을 받았다.', PARTIAL: '미약한 도움이 있었다.', FAIL: '도움을 거절당했다.' },
      SNEAK: { SUCCESS: '수상한 그림자가 스쳐 지나갔다.', PARTIAL: '인기척이 느껴진다.', FAIL: '수상한 자가 발각되었다.' },
      OBSERVE: { SUCCESS: '주의 깊은 관찰자가 목격되었다.', PARTIAL: '누군가 주변을 살피고 있었다.', FAIL: '감시자가 들켰다.' },
      INVESTIGATE: { SUCCESS: '누군가 무언가를 조사하고 있었다.', PARTIAL: '수색 흔적이 남아있다.', FAIL: '조사가 차단되었다.' },
    };
